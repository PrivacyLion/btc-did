...
- Kind 28250 (delegation_grant) event verification
- Schnorr signature verification (BIP-340)

Phase 26 Changes:
- KYC webhooks REMOVED (Persona/Jumio) — verification via three-gate genesis flow
- Server NOSTR signing REMOVED — server has zero NOSTR key involvement
- Kind 28201 (kyc_verification) REMOVED — no longer generated by server
"""
import os
import json
import time
...
KIND_ENROLLMENT_RESPONSE = 28202
KIND_DELEGATION_GRANT = 28250
KIND_REVOCATION = 28251

# DEPRECATED: KYC verification kind removed in Phase 26
KIND_KYC_VERIFICATION = 28201  # No longer used

# =============================================================================
# DEPRECATED (Phase 26) — KYC webhooks and server NOSTR signing removed
# =============================================================================
# These functions are no longer used:
# - KYC verification happens via three-gate genesis flow, not webhooks
# - Server has zero NOSTR signing involvement — all events signed by agents/humans
# - PERSONA_WEBHOOK_SECRET, JUMIO_WEBHOOK_SECRET, SIGNEDBYME_NOSTR_PRIVKEY env vars removed

def get_persona_webhook_secret() -> Optional[str]:
    """DEPRECATED: KYC webhooks removed in Phase 26."""
    logger.warning("get_persona_webhook_secret() deprecated — KYC webhooks removed in Phase 26")
    return None

def get_jumio_webhook_secret() -> Optional[str]:
    """DEPRECATED: KYC webhooks removed in Phase 26."""
    logger.warning("get_jumio_webhook_secret() deprecated — KYC webhooks removed in Phase 26")
    return None

def get_server_privkey() -> Optional[str]:
    """DEPRECATED: Server NOSTR signing removed in Phase 26."""
    logger.warning("get_server_privkey() deprecated — server NOSTR signing removed in Phase 26")
    return None

def get_server_pubkey() -> str:
    """DEPRECATED: Server NOSTR key removed in Phase 26."""
    logger.warning("get_server_pubkey() deprecated — server NOSTR key removed in Phase 26")
    return "0" * 64  # Return zeros to fail gracefully
...
# =============================================================================
# DEPRECATED (Phase 26) — Server KYC event creation removed
# =============================================================================
...
def create_kyc_verification_event(
    ...
    """
    DEPRECATED: KYC verification events removed in Phase 26.
    
    Identity verification now happens via three-gate genesis flow.
    Server has no NOSTR signing involvement.
    """
    logger.warning("create_kyc_verification_event() deprecated — KYC events removed in Phase 26")
    return None
...
def sign_event_id(event_id: str, privkey_hex: str) -> Optional[str]:
    ...
    All NOSTR events are signed by agents/humans, not the server.
    """
    logger.warning("sign_event_id() deprecated — server NOSTR signing removed in Phase 26")
    return None
...
# =============================================================================
# DEPRECATED (Phase 26) — KYC webhook verification removed
# =============================================================================
...
def verify_persona_webhook(signature_header: str, raw_body: bytes) -> WebhookVerifyResult:
    """DEPRECATED: KYC webhooks removed in Phase 26."""
    logger.warning("verify_persona_webhook() deprecated — KYC webhooks removed in Phase 26")
    return WebhookVerifyResult(valid=False, provider="persona", error="KYC webhooks removed in Phase 26")


def verify_jumio_webhook(signature_header: str, raw_body: bytes) -> WebhookVerifyResult:
    """DEPRECATED: KYC webhooks removed in Phase 26."""
    logger.warning("verify_jumio_webhook() deprecated — KYC webhooks removed in Phase 26")
    return WebhookVerifyResult(valid=False, provider="jumio", error="KYC webhooks removed in Phase 26")


# =============================================================================
# DEPRECATED (Phase 26) — Kind 28201 KYC events removed
# =============================================================================
...
def create_kyc_webhook_event(
    ...
    """
    DEPRECATED: KYC webhook events removed in Phase 26.
    
    Identity verification now happens via three-gate genesis flow.
    Server has no NOSTR signing involvement.
    """
    logger.warning("create_kyc_webhook_event() deprecated — KYC events removed in Phase 26")
    return None


# Legacy code kept for reference but no longer executed
def _create_kyc_webhook_event_legacy_DO_NOT_USE(
    nonce: str,
    passed: bool,
    attribute: str,
    provider: str,
    ...
    """Legacy function kept for reference only. DO NOT USE."""
    return None
    
    # Original implementation removed for Phase 26 cleanup
